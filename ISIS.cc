...
// - crs:           structure crs_ISIS, generated by Hcrs from crs_seed
...
void Prove_ISIS(uint8_t** Pi_ptr, const unsigned char* crs_seed, const CRS_t& crs, const IPK_t& ipk, const mat_zz_p& P, const mat_zz_p& C, const vec_zz_p& mex, const mat_zz_p& B_f, const vec_ZZ& Bounds, const long& aux, const Vec<vec_ZZ>& w0)
{
    ...
    size_t          len_a1, len_a2, len_c0, len_c1, len_idx_hid, len_mex, len_B_f, len_Bounds, len_aux, max_len;
    ...
    // NOTE: using crs_seed, ipk.a1, ipk.a2, ipk.c0, ipk.c1, idx_hid, instead of crs, P, C to speedup Hash_Init    
    ...
    // cout << "  Size Hash_Init: " << (SEED_LEN + len_a1 + len_a2 + len_c0 + len_c1 + len_idx_hid + len_mex + len_B_f + len_Bounds + len_aux)/1024.0 << " KiB" << endl; // 1 KiB kibibyte = 1024 bytes

    lengths = {SEED_LEN, len_a1, len_a2, len_c0, len_c1, len_idx_hid, len_mex, len_B_f, len_Bounds, len_aux};
    max_len = *max_element(begin(lengths), end(lengths)); 
    buffer = new uint8_t[max_len];
    
    state0 = Hash_Init(reinterpret_cast<const uint8_t*>(&crs_seed[0]), SEED_LEN);
    ...
    Hash_Update(state0, buffer, len_B_f);
...
long Verify_ISIS(const unsigned char* crs_seed, const CRS_t& crs, const IPK_t& ipk, const mat_zz_p& P, const mat_zz_p& C, const vec_zz_p& mex, const mat_zz_p& B_f, const vec_ZZ& Bounds, const long& aux, uint8_t** Pi_ptr)
{
    ...
    serialize_mat_zz_p(buffer, len_B_f, d0, t0, B_f);
    Hash_Update(state, buffer, len_B_f);
    serialize_vec_ZZ(buffer, len_Bounds, 2, Bounds);
