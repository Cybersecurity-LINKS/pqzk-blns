...
)
{
    ...
    old_times.reserve(runs);
    new_times.reserve(runs);
    ...
    for (int r = 0; r < runs; ++r)
    {
        if ((r % 2) == 0)
        {
            double t_0 = GetWallTime();
            old_fn(old_ctx);
            double t1 = GetWallTime();
            old_times.push_back(t1 - t_0);

            double t2 = GetWallTime();
            new_fn(new_ctx);
            double t3 = GetWallTime();
            new_times.push_back(t3 - t2);
        }
        else
        {
            double t2 = GetWallTime();
    ...
    for (double x : old_times) old_mean += x;
    for (double x : new_times) new_mean += x;

    old_mean /= old_times.size();
    new_mean /= new_times.size();
    ...
    for (double x : old_times)
    {
        double d = x - old_mean;
        old_std += d * d;
    }

    for (double x : new_times)
    {
        double d = x - new_mean;
        new_std += d * d;
    }

    old_std = std::sqrt(old_std / old_times.size());
    new_std = std::sqrt(new_std / new_times.size());

    const double diff = std::fabs(old_mean - new_mean);
    const double max_std = std::max(old_std, new_std);

    std::printf("\n=== BENCHMARK ===\n");
    std::printf("%s: avg = %.9f   std = %.9f\n", old_name, old_mean, old_std);
    std::printf("%s: avg = %.9f   std = %.9f\n", new_name, new_mean, new_std);

    if (max_std > 0.0)
    {
        std::printf("diff / max_std = %.3f\n", diff / max_std);
    }

    if (max_std > 0.0 && diff < max_std)
    {
        std::printf("Result: SAME SPEED / noise dominated\n");
    }
    else if (old_mean < new_mean)
    {
        double speedup = (new_mean / old_mean - 1.0) * 100.0;
        std::printf("Result: %s faster by %.2f%%\n", old_name, speedup);
    }
    else
    {
        double speedup = (old_mean / new_mean - 1.0) * 100.0;
        std::printf("Result: %s faster by %.2f%%\n", new_name, speedup);
    }
}
//==============================================================================
